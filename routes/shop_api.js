...
router.get('/:apikey/:shop_id', async (req, res) => {
    try {
        ...
        const [shop] = await db.query('SELECT DISTINCT shop.*, GROUP_CONCAT(DISTINCT tag_list.tag_name) AS tag_names, GROUP_CONCAT(DISTINCT shop_images.image) AS shop_images FROM shop JOIN tag ON shop.id = tag.shop_id JOIN tag_list ON tag.tag_id = tag_list.id LEFT JOIN shop_images ON shop.id = shop_images.shop_id WHERE shop.id = ?', [shop_id]);
